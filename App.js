...
export default function App() {
	...
	useEffect(() => {
		...
		AsyncStorage.getItem(StorageKeys.LANGUAGE_STORAGE_KEY).then((storedValue) => {
			if (storedValue != null) {
				setCurrentLanguage(TranslationManager.getLanguageObject(storedValue));
			} else {
				AsyncStorage.setItem(StorageKeys.LANGUAGE_STORAGE_KEY, AvailableLanguages.ENGLISH).then(() => {
					setCurrentLanguage(TranslationManager.getLanguageObject(AvailableLanguages.ENGLISH));
				});
			}
			languageLoaded = true;
		});

		AsyncStorage.getItem(StorageKeys.SHOW_INTRODUCTION_KEY).then((storedValue) => {
			if (storedValue != null) {
				setShowIntroduction(JSON.parse(storedValue));
			}
		});

		AsyncStorage.getItem(StorageKeys.SHOW_PERSONAL_ADS_KEY).then((storedValue) => {
			if (storedValue != null) {
			...
			<SafeAreaView>
				...
				<View style={AppStyle.introductionWrapper}>
					...
					<View style={AppStyle.introductionDescriptionWrapper}>
						<Text style={AppStyle.introductionDescription}>{item.description}</Text>
					</View>
				</View>
			</SafeAreaView>
		);
	...
	const RenderNextButton = () => {
		return (
			<View>
				<Text style={AppStyle.introductionText}>{currentLanguage.introductionNext}</Text>
			</View>
		);
	};
	...
	const RenderFinishButton = () => {
		return (
			<View>
				<Text style={AppStyle.introductionText}>{currentLanguage.introductionDone}</Text>
			</View>
		);
	};
	...
	const RenderSkipButton = () => {
		return (
			<View>
				<Text style={AppStyle.introductionText}>{currentLanguage.introductionSkip}</Text>
			</View>
		);
	};

	const introFinished = () => {
		AsyncStorage.setItem(StorageKeys.SHOW_INTRODUCTION_KEY, JSON.stringify(false)).then(() => {
